...
function loadFiles(ev){
    ...
    Promise.all(readers).then((values) => {
		...
		for(var i = 0; i < values.length; i++){
			...
			for(var j = 0; j < headTag.length; j++){									//check for single file mode
			...
			if(isEncrypted){													//call encryption or decryption depanding on how the file starts
				fileOutName = fileOutName.slice(0,-6);
				if(fileInBin[headTag.length] == 0){								//check byte right after head tag; 0 for folder mode
					decrypt(fileInBin,fileOutName,true)								//folder mode; symmetric
				}else{

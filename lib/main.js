...
function resetKeys(){
	...
	fileIn.classList.remove("fileReady");
	fileIn.classList.add("fileInit");
...
function acceptPwd(){
	...
	setTimeout(function(){
		...
		if(myIndex == -1){
			...
			var publicKey = document.createTextNode(myEzLock);
...
function encrypt(fileInBin, fileOutName){
	...
	setTimeout(function(){										//delay to allow blinker to start
		...
		if(isKeyFile){
			...
			fileIn.classList.remove("fileInit");
			fileIn.classList.add("fileReady");

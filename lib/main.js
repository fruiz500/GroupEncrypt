...
//add event listeners and some processing of users in GroupKeys.json
window.onload = function() {

	//fetch GroupKeys.json and process it. Adapted from https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Client-side_web_APIs/Fetching_data

	// Call `fetch()`, passing in the URL.
	fetch('GroupKeys.json')
	...
	// the text, and we extract the data from it.
	.then((text) => {
		window.GroupKeys = JSON.parse(text);				//make object containing settings and public keys and other global variables
		window.GroupName = GroupKeys['GroupName'];
		window.headTag = JSON.parse(GroupKeys['headTag']);
		...
		admin.addEventListener('click', function(){window.open('admin.html')});
	...
	.catch((error) => userList.textContent = `Could not fetch data: ${error}`);
}

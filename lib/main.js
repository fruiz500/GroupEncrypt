...
function acceptPwd(){
	...
	setTimeout(function(){
		...
		pwdMsg.textContent = 'Password accepted; public key displayed by checkbox below';
		pwdBox.value = '';																		//all done, so empty the password box
		checkArea.style.display = 'block';
		fileArea.style.display = 'block';
		radioArea.style.display = 'inline';
		fileIn.disabled = false;
		fileMsg.textContent = 'Click this button to load a file, or drag the file onto it';
...
function loadFile(){
	...
	fileReader.onload = function(fileLoadedEvent){
		...
		var isEncrypted1 = true,													//check that the file begins with the encrypt marker
			isEncrypted2 = false;
		for(var i = 0; i < headTag1.length; i++){									//check for single file mode
			if(fileInBin[i] != headTag1[i]){
				isEncrypted1 = false;
				break
			}
		}
		if(!isEncrypted1){															//not single file mode, maybe it's folder mode
			var isEncrypted2 = true;
			for(var i = 0; i < headTag1.length; i++){
				if(fileInBin[i] != headTag2[i]){
					isEncrypted2 = false;
					break
				}
			}
		}
		if(isEncrypted1){													//call encryption or decryption depanding on how the file starts
			fileOutBox.value = fileName.slice(0,-6);
			decrypt(false)													//single file mode; asymmetric
		}else if(isEncrypted2){
			fileOutBox.value = fileName.slice(0,-6);						//folder mode; symmetric
			decrypt(true)
		}else{
			fileOutBox.value = fileName + '.crypt';
			encrypt(false)													//not a Key File; encryption mode determined by radio buttons
		}
    };
...
//makes a key file, by encrypting an empty file in Signed mode
function makeKeyFile(){
	if(!confirm("Only the Administrator should make new Folder Key files. Are sure you want to continue?")) return;
	fileInBin = [];
	encrypt(true)
}
...
if(folderMode.checked){										//folder mode: symmetric encryption using folderKey as key
	
	isKeyFile = false;				//to get correct messages at the end
	if(!folderKey){
	...
	fileOutBin = concatUi8([headTag2,nonce,msgKeyCipher,cipher]);		//start with header v2 and the nonce, next is msgKey encrypted with the folder Key, and the encrypted file data
	...
	fileOutBin = concatUi8([headTag1,recipients,nonce,myLock.slice(0,8)]);	//global output starts with header v1, No. of recipients, 24-byte nonce, first 8 bytes of sender's public Key			
...
fileMsg.style.color = 'green';
	if(isKeyFile){						//not available in symmetric mode
		fileMsg.textContent = 'Folder Key file created; You can now encrypt files under this key';
		pwdMsg.textContent = 'Folder Key file created; You can now encrypt files under this key';
		folderMode.checked = true;
		folderKey = msgKey;
		saveFileOut('.FolderKey.crypt')
	}else if(showOutNameCheck.checked){
		...
		saveFileOut(fileOutBox.value)							//download automatically if the Save button is not showing
	}
},20)
}

function decrypt(isFolderMode){
	if(!fileInBin) return;
	if(!refreshKey()) return;			//check that the Key is active and stop if not
	if(locks.length <= 0) return;
...
if(isFolderMode){																//symmetric mode decryption
	...
	var nonce = fileInBin.slice(headTag2.length,headTag2.length+24),			//24 bytes
		msgKeyCipher = fileInBin.slice(headTag2.length+24,headTag2.length+72),	//encrypted key, 48 bytes
		cipher = fileInBin.slice(headTag2.length+72);							//rest of it; encrypted file
	...
	fileOutBin = nacl.secretbox.open(cipher,nonce,msgKey);								//main file decryption
	...
	if(!fileOutBin){												//decryption failed
		...
		if(showOutNameCheck.checked){
			fileMsg.textContent = 'Decryption successful. Edit file name if needed and save it with the button'
		}else{
	...
	var	recipients = fileInBin[headTag1.length],								//number of recipients. '0' reserved for special cases
		cipherArray = new Array(recipients),
		stuffForId = myLock,
		isLegacy = false;

	var nonce = fileInBin.slice(headTag1.length+1,headTag1.length+25),		//24 bytes
		lockID = fileInBin.slice(headTag1.length+25,headTag1.length+33),		//first 8 bytes of sender's public key
		cipherInput = fileInBin.slice(headTag1.length+33);					//rest of it; contains IDtags + encrypted message keys, and encrypted file
	...
	if(cipher.length == 0){												//encrypted folder Key
		...
		fileMsg.textContent = 'Folder Key decrypted. You are now set to encrypt and decrypt files from this folder. Last encrypted by ' + sender;
		folderMode.checked = true

	}else{																//asymmetric-encrypted file
		fileOutBin = nacl.secretbox.open(cipher,nonce,msgKey);						//decrypt the main message; false if error
...
function saveFileOut(name){
	if(fileOutBin) downloadBlob(fileOutBin, name, 'application/octet-stream')
}
...
window.onload = function() {
	...
	saveBtn.addEventListener('click', function(){saveFileOut(fileOutBox.value)});

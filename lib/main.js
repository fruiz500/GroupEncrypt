...
const lockListfileInName = 'GroupKeys.txt';							//file containing public keys
    ...
    Promise.all(readers).then((values) => {
		...
		for(var i = 0; i < values.length; i++){
			...
			var isEncrypted1 = true,													//check that the file begins with the encrypt marker
				isEncrypted2 = false;
			for(var j = 0; j < headTag1.length; j++){									//check for single file mode
				if(fileInBin[j] != headTag1[j]){
					isEncrypted1 = false;
					break
				}
			}
			if(!isEncrypted1){															//not single file mode, maybe it's folder mode
				...
				for(var j = 0; j < headTag1.length; j++){
					if(fileInBin[j] != headTag2[j]){
						isEncrypted2 = false;
						break
					}
				}
			...
			if(isEncrypted1){													//call encryption or decryption depanding on how the file starts
				...
				decrypt(fileInBin,fileOutName,false)								//single file mode; asymmetric
			}else if(isEncrypted2){
				fileOutName = fileOutName.slice(0,-6);								//folder mode; symmetric
				decrypt(fileInBin,fileOutName,true)
			}else{
				fileOutName = fileOutName + '.crypt';
				encrypt(fileInBin,fileOutName,false)								//not a Key File; encryption mode determined by checkbox
			}
		}
    });

	fileIn.type = '';													//reset file input
	fileIn.type = 'file'
}
...
function makeKeyFile(){
	if(folderKey){
		var alertMsg = "Only the Administrator should update Folder Key files. Are sure you want to continue?"
	}else{
		var alertMsg = "Only the Administrator should make new Folder Key files. Are sure you want to continue? A new Folder Key makes files encrypted with a previous key undecryptable"
	}
	if(!confirm(alertMsg)) return;

	encrypt([],'_FolderKey.crypt',true)
}
...
function encrypt(fileInBin, fileOutName, isKeyFile){
	...
	setTimeout(function(){										//delay to allow blinker to start
		...
		if(!singleMode.checked && !isKeyFile){						//folder mode: symmetric encryption using folderKey as key
			...
			var fileOutBin = concatUi8([headTag2,nonce,msgKeyCipher,cipher]);		//start with header v2 and the nonce, next is msgKey encrypted with the folder Key, and the encrypted file data
...
function decrypt(fileInBin, fileOutName, isFolderMode){
	if(!fileInBin) return;
	if(!refreshKey()) return;			//check that the Key is active and stop if not
	if(locks.length <= 0) return;
...
var locksSelected = [], usersSelected = [];		//same, but only for selected users; excludes lecagy users
...
function updateUsers(){
	...
	for(var i = 0; i < usersSelected.length; i++){					//fill locks array
		var lock64 = changeBase(GroupKeys[usersSelected[i]],base36,base64);
		while (lock64.length < 43) lock64 = 'A' + lock64;					//prepend zeros to get correct length
		locksSelected.push(nacl.util.decodeBase64(lock64))
	}

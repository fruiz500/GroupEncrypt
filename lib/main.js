...
function acceptPwd(){
	...
	setTimeout(function(){
		myKey = wiseHash(key,'');									//no salt is used
		myLock = makePub(myKey);									//and its matching public key

		textBox.textContent = changeBase(nacl.util.encodeBase64(myLock).replace(/=+$/,''), base64, base36);	//for display as text; easy to dictate
		pwdMsg.style.color = '';
...
//crypto functions; similar to Anonymous mode in PassLok
function encrypt(){
	if(!fileInBin) return;
...
setTimeout(function(){
	...
	var	msgKey = nacl.randomBytes(32),	//message key for symmetric encryption
		...
		secdum = nacl.randomBytes(32),	//make dummy Key and public key
		pubdum = makePub(secdum);

		fileOutBin = concatUi8([headTag,recipients,nonce,pubdum]);	//global output starts with header, No. of recipients, 24-byte nonce, dummy public Key			
	...
	for (index = 0; index < locks.length; index++){
		var sharedKey = makeShared(locks[index],secdum),
...
setTimeout(function(){
	...
	var nonce = fileInBin.slice(headTag.length+1,headTag.length+25),		//24 bytes
		pubdum = fileInBin.slice(headTag.length+25,headTag.length+57),		//retrieve ephemeral lock
		cipherInput = fileInBin.slice(headTag.length+57);					//rest of it; contains IDtags + encrypted message keys, and encrypted file
...
function fillList(){
	...
	for(var name in GroupKeys){
		...
		if(isLock(GroupKeys[name])){					//make array just with public keys in Uint8 format
			locks.push(nacl.util.decodeBase64(changeBase(GroupKeys[name].trim().replace(/l/g,'L'),base36,base64)));		//make capital 'L' in case it was smallcase
			users.push(name)
		}
	}

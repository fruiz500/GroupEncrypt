...
function acceptPwd(){
	...
	//now make the binary secret Key from the password. Do it for the Edwards curve first for compatibility with PassLok
	setTimeout(function(){
		myKey = nacl.sign.keyPair.fromSeed(wiseHash(key,groupName)).secretKey;		//make the Edwards curve secret key first; global variable groupName is used as salt
        myKeyDH = ed2curve.convertSecretKey(myKey);							//and then the Montgomery curve key										
		myLock = nacl.sign.keyPair.fromSecretKey(myKey).publicKey;			//matching public key
...
function encrypt(fileInBin, fileOutName){
	...
	var padding = crypto.getRandomValues(new Uint8Array(100));		//for compatibility with PassLok; no hidden message; 100 bytes
	...
	setTimeout(function(){										//delay to allow blinker to start
		...
		if(folderMode.checked && !isKeyFile){						//folder mode: symmetric encryption using folderKey as key
			...
			for (i = 0; i < locksShuffle.length; i++){
				var sharedKey = makeShared(convertPub(locksShuffle[i]),myKeyDH),								//use encrypter's private key: signed mode
...
//				padding = fileInBin.slice(headTag.length+25,headTag.length+125),		//not really used
...
//				padding = fileInBin.slice(headTag.length+25,headTag.length+125),		//not really used
			...
			var	sharedKey = makeShared(convertPub(locks[index]),myKeyDH)
...
var groupName = GroupKeys['GroupName'];
...
//in case the group name is edited
function changeGroupName(){
	groupName = groupNameBox.textContent.trim()
}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	groupNameBox.addEventListener('keyup',changeGroupName);
